...
.DESCRIPTION
    ...
    - SHELL-ENABLED: ESX Shell service is currently running
    - FIREWALL-OPEN: SSH allowed through the ESX firewall
...
process {
    ...
    foreach ($ESXi in $HostsToScan) {
        ...
        # Query host services once; filter for SSH and ESX Shell
        $HostServices = Get-VMHostService -VMHost $ESXi
        $SSHService = $HostServices | Where-Object { $_.Key -eq "TSM-SSH" }
        $ShellService = $HostServices | Where-Object { $_.Key -eq "TSM" }
        ...
        # Check whether the firewall allows SSH
        $SSHFirewall = Get-VMHostFirewallException -VMHost $ESXi -Name "SSH Server" -ErrorAction SilentlyContinue
        if ($null -eq $SSHFirewall) {
            Write-Warning "Could not read the SSH Server firewall exception on $($ESXi.Name); FIREWALL-OPEN flagging is incomplete for this host."
        }
        elseif ($SSHFirewall.Enabled) {
            $Flags += "FIREWALL-OPEN"
        }
        ...
        $Result = [PSCustomObject]@{
            ...
            SSHFirewallOpen = if ($null -ne $SSHFirewall) { $SSHFirewall.Enabled } else { "Unknown" }
    ...
    Write-Host "Hosts audited:          $ScannedCount"
