...
Run this check on the vCenter appliance and other Linux-based appliances:

```bash
# Check environment variables of ALL running processes
# (Note: 'env' only shows the current shell - this checks every process)
# /proc/$pid/environ holds null-separated environment variables;
...
> **VMware ESX Note**: This check cannot be run on VMware ESX hosts. The ESX kernel does not expose per-process `/proc/<pid>/environ` entries, so the environment of a running process is not readable from the ESX shell. Run this check on the vCenter appliance and other Linux-based appliances, where BRICKSTORM's environment variable markers apply.
...
|-----------|------------------|--------------------|
| **Binary Path** | `/bin/vvold` or `/usr/lib/vmware/vvold/bin/vvold` | `/tmp/`, `/vmfs/volumes/`, or other unexpected path |
...
# (ESX ships its own lsof that does not support -p, and pidof is
# not available on ESX, so filter the full listing instead)
lsof | grep vvold
# LEGITIMATE paths: /bin/vvold or /usr/lib/vmware/vvold/bin/vvold
...
# Compare against the consolidated hash list in scripts/brickstorm-hashes.sha256
# and the current hash lists in CISA AR25-338A (see Official IOC Sources below)
```
