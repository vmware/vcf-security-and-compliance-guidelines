...
See [Rule 8: Disk Swap and Snapshot Attack](#rule-8-disk-swap-and-snapshot-attack-vcenter-t1003003) for detailed detection logic and SIEM query examples.

### VM Migration Events - Pre-Clone Staging (vCenter)
...
The VMware ESX audit log does not record VM migrations. This activity is visible only in the vCenter event database, so these detections require vCenter event monitoring.
...
eventTypeId:(VmMigratedEvent OR VmRelocatedEvent) AND vm.name:(*DC* OR *domain* OR *vault* OR *CA*)
```
...
These host login-failure events are relayed through the vCenter event system (see [Section 14](#14-vcenter-relayed-esx-events)), not written to the ESX audit log. The ESX audit log records the corresponding lockout as `account.locked` (Section 3).
...
Any `system.update.add` event outside of scheduled maintenance windows should trigger immediate investigation.
...
UNC5221 deploys specialized implants (Junction, GuestConduit) that use VSOCK to bypass network security.

### No Audit Events for Socket Activity

The ESX audit log does not record generic socket listen or connect activity, so the Junction listener on port 8090 and GuestConduit VSOCK traffic generate no audit events by themselves. Indirect audit evidence may appear if the attacker opens the port through the firewall (`network.fw.rule.enable`, or `esx.audit.net.firewall.config.changed` via vCenter, Section 14).

### Detection Commands for Junction/GuestConduit

Because these implants do not generate audit events, use active detection:
...
# Requires an iproute2 build with AF_VSOCK support; ss -x lists
# only AF_UNIX sockets and will NOT show VSOCK listeners
ss --vsock -l
```
...
```
object:(vm OR account OR service OR system OR network OR time OR shell OR ssh) AND result:success
```
...
```
object:(configenc OR network) AND result:success
```
Filter the `network` results for `network.fw.*` events (firewall changes).
