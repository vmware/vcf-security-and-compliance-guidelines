...
This reference describes the audit records that VMware ESX generates for security-related system events, including the parameters of each event. You can configure ESX to store audit records locally, transmit them to remote collectors, or both.
...
Audit records are sent to the ESX vmsyslogd daemon for processing. Each record has a timestamp, the name of the reporting process, the PID of the reporting process, a syslog facility of 13 (audit), and a syslog severity (level) that is appropriate for the record. If enabled, the vmsyslogd daemon stores audit records locally and transmits them to external collectors (remote hosts). When transmitted, audit records also contain the name of the system from which they originated.
...
Specifies the capacity of the audit record storage directory, in MiB. **You cannot decrease this allocation once set**. However, you can increase it at any time. The default is 4 MiB. VMware recommends 100 MiB.

## Syslog.global.auditRecord.storageEnable

Enables the local storage of audit records, into the directory specified by Syslog.global.auditRecord.storageDirectory. Configure the storage directory and the storage capacity before you enable audit logging; the storage directory cannot be changed while audit logging is enabled.
...
Use VMware PowerCLI to change these settings across multiple hosts. As an example, this command changes a specific host’s log host setting:
