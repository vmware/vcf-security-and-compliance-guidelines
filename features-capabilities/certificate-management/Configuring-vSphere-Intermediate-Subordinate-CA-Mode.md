...
With public certificate lifetimes limited to 200 days as of March 15, 2026, and dropping further to 100 days on March 15, 2027 and 47 days on March 15, 2029, the replacement of SSL/TLS certificates on infrastructure components now requires significantly more staff time than before: twice-yearly renewals today, on the way to monthly. To alleviate this burden on administrators, VMware vSphere includes a built-in feature known as the "VMware Certificate Authority." This feature, integrated into VMware vCenter, issues certificates within a vSphere cluster. By default, this Certificate Authority (CA) uses a self-signed CA root created during the initial vCenter installation. However, this CA root can be replaced with an intermediate CA certificate, signed by a trusted CA, in a method VMware designates as "Intermediate CA" mode.

Intermediate CA mode, also referred to as Subordinate CA mode, automates the deployment of trusted certificates to vSphere infrastructure components. Nonetheless, some organizations express concerns about issuing signing certificates in this manner, primarily due to potential impersonation of the organization and the possible lifespan of CA certificates. Mitigation strategies include the use of intermediate CA roots, certificate revocation lists, and even the creation of separate CA roots which are not generally trusted by organizational systems. However, these mitigations are beyond the scope of this document.

This document is not a substitute for the product documentation. Instead, it consolidates guidance and tips for making this change. Please note that our test environments may not mirror your production environments, so we strongly recommend testing this change in your own non-production environment. Testing lets you practice the process and verify your key and certificate material. Nested vSphere environments are excellent for this purpose, and they can be easily restored using snapshots. VMware Hands-On Labs might also be suitable for this type of exercise. There's no rule saying you must adhere strictly to the lab script!
...
This process was tested with both VMware vSphere 7 Update 3m and VMware vSphere 8 Update 1, using two vCenter instances in Enhanced Linked Mode, each with three hosts configured for DRS, vSphere HA, vSAN, and distributed virtual switches. The following should be confirmed in your own environment prior to starting work:
...
*   SSH access to vCenter has been enabled for this work and you have access to the root login or equivalent.
*   You have access to the administrator@vsphere.local login for the duration of this work.
*   You have a recent backup of vCenter.

As always, applying the latest updates for your supported major version of vSphere is recommended.

### DRS and HA

While this process is tested and works smoothly on clusters with DRS, HA, and vSAN enabled, planning for the possibility of something going awry means a faster recovery if it happens. In that light, consider temporarily placing DRS in “partially automated” mode and deactivating vSphere HA, to avoid complications and delays waiting for cluster elections (especially in larger clusters). Do not deactivate DRS completely as it will remove all defined resource pools.

### External Systems

Other components which connect to vSphere may need to have their connections reestablished after the certificates are replaced. Temporarily block or remove access for service accounts for external tools and systems, to prevent disruptions to those systems as the cluster is updated, and to help confirm the reconnections afterwards.

### Administrative Workstation

Install two web browsers on the workstation you plan to use, to help determine where a trust issue might lie. It is likely that this work will confuse the HSTS protections of browsers and require “forgetting” the web site. The methods for this vary. For example, Mozilla Firefox allows you to forget a site with Ctrl-Shift-H (view History), then right-clicking on the site and choosing “Forget About This Site.” Google Chrome also has the “thisisunsafe” workaround, if needed.

Install the trusted root CA certificates on the workstation, and verify that it can access vCenter and host management interfaces via HTTPS and SSH, if necessary. Install OpenSSL on the workstation for troubleshooting and certificate & key manipulation.

PowerCLI examples are given here and recommended for automation. Install and configure PowerCLI on your workstation. This document assumes some familiarity with PowerCLI, but if you’re new to it, run “Connect-viserver -server vcenter-1.7.fcotr.org” (using whatever your vCenter FQDN is) before you try anything.
...
2.  To prevent dependency issues with vSphere startup the intermediate CA key should not be protected with a password for the import operation. You can use a command like “openssl rsa -in vsphere-key.pem -out vsphere-key-nopw.pem” to write a copy of the key file without the password. You may encounter FIPS errors with OpenSSL on the vCenter Server Appliance if you try this there, hence the recommendation to have a copy of OpenSSL elsewhere. Delete the unprotected key file at the end of this process.
...
It is sometimes easier to SSH to the vCenter Server Appliance, enter the shell, then initiate the scp outbound using the vCenter shell itself. Otherwise for incoming scp you will need to change the root shell on the VCSA using the “chsh -s /bin/bash root” command from the shell itself.

vCenter Preparation
-------------------

vCenter has advanced parameters that set the issued certificate CN data:
...
Repeat these steps for all vCenter instances you are working on:
...
From here you will execute /usr/lib/vmware-vmca/bin/certificate-manager on one vCenter at a time. If something goes wrong you can use Certificate Manager option 8 to restore your original certificate configurations and start again.
...
4.  Enter a VCF SSO administrator username (the default of administrator@vsphere.local works, just press enter if you plan to use that)
5.  Enter the password for the account you specified.
6.  You will be prompted for all the same information as what you set above, for vCenter. Set it the same way. Hint: you may discover that the shell does not like backspace when entering data (you get ^H). Try Ctrl-Backspace instead, or if you’re really stuck use Ctrl-C and start again.
    ...
    9.  Enter proper value for 'Hostname' (Provide comma separated values for multiple Hostname entries) \[Enter valid Fully Qualified Domain Name(FQDN), For Example : example.domain.com\] – (**We entered the vCenter FQDN**, so for this example we used “vcenter-1.7.fcotr.org”)
...
9.  Certificate Manager will now replace all the certificates for vCenter services. Wait patiently until it is done. This may take some time as it will restart all vCenter services (same as if you’d rebooted the VCSA).
...
11.  When it completes you will be logged out of the web-based vSphere Client. Log back in and verify that the certificate is what you expect and no new alarms are present. If you did not disable vSphere HA you may want to wait for cluster elections to finish before proceeding with ESX certificate replacements.
...
12.  Repeat this process as needed or desired for other linked vCenter instances.

ESX Certificates
----------------

Once the VMCA is operational and the vSphere Client shows the correct certificates, update the ESX host certificates.
...
2.  Click the “Refresh CA Certificates” button for that host. Verify in Recent Tasks that the operation completed (you may need to refresh the client if you don’t see it). If it did not succeed then troubleshoot the error.
3.  Now click the “Renew” button. You will be prompted to confirm the renewal. Look at the Recent Tasks to see it complete. You may need to refresh the vSphere Client to see the new certificate data, which should reflect the information in the vCenter advanced parameters.
...
If you are connected to multiple vCenter instances the Get-VMHost cmdlet will return all relevant hosts. Hosts may temporarily show as “Not Responding” in the vSphere Client while this process is happening. If a host remains in the “Not Responding” state, directly verify that the host client is responding, then try reconnecting it (Connection->Connect).
...
*   Verify that you have a secure copy of the CA files, then clean up files left on the VCSA.
