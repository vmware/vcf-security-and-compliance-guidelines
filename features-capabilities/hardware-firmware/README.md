...
Hardware firmware updates routinely address confidentiality, integrity, and availability issues. Broadcom strongly recommends that customers maintain current hardware firmware on their systems. vSphere Lifecycle Manager can work in concert with system management tools from our hardware partners to update and manage both the ESX system image configuration and the hardware firmware revisions simultaneously. For more information see the vSphere product documentation.

## CPU Vulnerabilities and ESX Schedulers

Exploitation attempts against processor-level vulnerabilities are difficult to detect with standard security scanning tools. Unlike software, CPU hardware cannot be easily modified after manufacturing and deployment in servers or workstations.

While VMware ESX does not cause these vulnerabilities, its CPU scheduler can help protect against some exploits by helping to control the use of HyperThreading or Symmetric Multi-Threading (SMT). For example, the L1TF vulnerability showed that Intel CPU cores could leak data between threads. The ESX Side-Channel Aware Scheduler (SCAv1) addressed this by disabling Intel Hyper-Threading, which improved security but reduced CPU capacity. The MDS vulnerability presents similar data leakage risks between processes, and SCAv1 mitigates those in the same way. In environments with different security requirements, you may determine that using Hyper-Threading within virtual machines aligns with your security policy. For these cases, SCAv2 enables Hyper-Threading use, which helps recover some performance and capacity, consistent with that security policy.
...
For example, an attacker exploiting these vulnerabilities could read your Chief Financial Officer's password-protected financial projections from memory.
...
The performance impacts of these remediations affect every workload differently. That said, SCAv1 worst case performance loss can reach 30%, and SCAv2 10%, depending on the workload. If you lack CPU headroom in your cluster and cannot tolerate slower workloads, consider the following options.

First, while you can enable different schedulers on different hosts within a cluster, it is not a good idea to do so. Always enable the same scheduler on all hosts in a cluster. Having differing schedulers causes confusion, configuration errors, and inconsistent security and performance behavior. That said, a valid approach might be to sequester workloads that need the per-process protections of SCAv1 on a separate cluster and run SCAv2 on other clusters that are compatible with the workloads’ risk.
...
Third, if you have enabled Enhanced vMotion Compatibility (EVC) in your clusters it is easy to add additional capacity. Simply acquire additional capacity and add it to the cluster. EVC is a useful tool that is often overlooked when building clusters. New CPU generations typically add instructions and features that prevent vMotion from moving VMs between old and new servers; EVC masks those differences so vMotion continues working. Later, when the whole cluster is upgraded and old equipment retired, the EVC level can be changed to support the newer CPUs, and VMs can take advantage of any newer CPU features. In practice, most enterprise workloads do not take advantage of new CPU features, and the benefit EVC brings in upgrade and expansion flexibility usually outweighs the small performance differences.

Fourth, if your hardware is aging there might be other benefits to considering a refresh. Newer hardware performance and sizing options can decrease server counts, which in turn saves data center space, power, cooling, port count, and other infrastructure costs. A refresh is also an opportunity to use vSAN and its data-at-rest & stretched cluster capabilities.
...
### Q1: Are these types of vulnerabilities specific to VMware software?
...
No. Security needs vary by organization. Evaluate business risk, cost, and potential legal implications. This decision requires input from key stakeholders (Chief Information Security Officer, Chief Financial Officer, Chief Information Officer, Chief Executive Officer).
...
Update VMware vCenter first, then ESX hosts. This sequence supports proper Enhanced vMotion Compatibility (EVC) and Distributed Resource Scheduler (DRS) coordination.
