...
## SCG Control IDs in Output and Control Selection

Every check is tagged with the SCG control ID it implements, both in the code and in the output. Output lines carry the control ID in brackets after the level, and the CSV output includes a `Control` column, so results can be matched directly against the controls spreadsheet:

```text
[2026-06-12 10:15:01] [FAIL] [esx-9.tls-ciphers] esx-01a.vcf.lab: TLS profile is not configured correctly (COMPATIBLE)
```

The per-object scripts accept `-Controls` and `-ExcludeControls` to run a subset of checks by SCG control ID:

```powershell
.\audit-esx-9.ps1 -Name esx-01a.vcf.lab -Controls esx-9.ssh,esx-9.tls-ciphers
.\remediate-esx-9.ps1 -Name esx-01a.vcf.lab -Controls esx-9.dcui-timeout
```

When neither parameter is specified, all checks run. Requesting a control ID the script does not cover logs a WARNING. A few closely related network checks run as a group: selecting any standard-switch or distributed-switch control runs that switch section, with each output line still tagged with its specific control ID. Remediation options that already require an opt-in switch (such as `-RemediateStandardSwitches`, `-EnableLockdownMode`, and `-RemediateTLSCiphers`) still require that switch in addition to any `-Controls` selection.
...
These tools assume the versions of the products associated with this guide (VMware Cloud Foundation 9.1.0). Using these tools against a different environment will have untested results.
...
| `-Controls` | No | Limit the run to these SCG control IDs (comma-separated, e.g. `esx-9.ssh,esx-9.tls-ciphers`). Default: all controls |
| `-ExcludeControls` | No | Skip these SCG control IDs |
...
| `-RemediateTLSCiphers` | No | Enable the NIST_2024_TLS_13_ONLY TLS profile (TLS 1.3 only, modern ciphers). This will require a host reboot. |
...
| `-RemediateTLSCiphers` | No | Set the vCenter TLS profile to NIST_2024_TLS_13_ONLY (TLS 1.3 only, modern ciphers). This briefly restarts vCenter services. |
...
| `-UpdateHardwareVersion` | No | Updates the virtual machine hardware version to the host's current maximum. There may be compatibility considerations for your guest operating system. |
